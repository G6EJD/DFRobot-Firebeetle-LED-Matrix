...
DFRobot_HT1632C::DFRobot_HT1632C(byte data, byte wr, byte RD, byte cs){
	data_t = data;
	cs_t = cs;
	wr_t = wr;
	rd_t = rd_t;
...
DFRobot_HT1632C::DFRobot_HT1632C(byte data, byte wr, byte cs){
	data_t = data;
	cs_t = cs;
	wr_t = wr;
...
void DFRobot_HT1632C::writeCommand(byte cmd){
	uint16_t str = 0;
...
void DFRobot_HT1632C::writeBits(uint16_t data, byte length){
	pinMode(data_t, OUTPUT);
...
void DFRobot_HT1632C::writeRAM(byte addr, byte data){
...
void DFRobot_HT1632C::setPixel(byte x, byte y){
	if((x<0) || (x>=width) || (y<0) || (y>=height))	return;
	
	matrix[x] |= 0x80>>y;
}

void DFRobot_HT1632C::clrPixel(byte x, byte y){
	if((x<0) || (x>=width) || (y<0) || (y>=height))	return;
	
	matrix[x] &= ~(0x80>>y);
}

void DFRobot_HT1632C::setPwm(byte value){
	if(value > 15) value = 15;
			writeCommand(DFROBOT_HT1632_PWM_CONTROL | value);
}

void DFRobot_HT1632C::drawLine(byte xStart, byte yStart, byte xStop, byte yStop){
...
void DFRobot_HT1632C::clrLine(byte xStart, byte yStart, byte xStop, byte yStop){
	int16_t steep = abs(yStop - yStart) > abs(xStop - xStart);
  if (steep) {
    _swap_int16_t(xStart, yStart);
    _swap_int16_t(xStop, yStop);
...
void DFRobot_HT1632C::drawImage(const byte * img, byte width_t, byte height_t, byte x, byte y, int img_offset){
	byte bytesPerColumn = (height_t >> 3) + ((height_t & 0b111)?1:0);
	...
	while(src_x < width_t){
		...
		while(src_y < height_t){
			...
			byte copyInNextStep = 8 - max((src_y & 0b111), (dst_y & 0b111));
			copyInNextStep = min(copyInNextStep, (height - src_y));
			byte dst_copyMask = (0b1 << copyInNextStep) - 1;
			dst_copyMask <<= (8 - (dst_y & 0b111) - copyInNextStep);
			byte copyData = pgm_read_byte(&img[img_offset + (bytesPerColumn * src_x) + (src_y >> 3)]) << (src_y & 0b111);
...
void DFRobot_HT1632C::drawImageStr(const byte * img, byte width_t, byte height_t, byte x, byte y, int img_offset){
	byte bytesPerColumn = (height_t >> 3) + ((height_t & 0b111)?1:0);
	if(y + height_t < 0 || x + width_t < 0 || y > height || x > width){
		return;
	}
...
void DFRobot_HT1632C::drawText(const char text [], int x, int y, const byte font [], int font_end [], byte font_height, byte gutter_space){
	int curr_x = x;
	unsigned int i = 0;
	char currchar;
...
void DFRobot_HT1632C::drawTextStr(const char text [], int x, int y, const byte font [], int font_end [], byte font_height, byte gutter_space){
	int curr_x = x;
	byte i = 0;
	char currchar;
...
int DFRobot_HT1632C::getCharWidth(int font_end [], byte font_height, byte font_index) {
	byte bytesPerColumn = (font_height >> 3) + ((font_height & 0b111)?1:0); 
...
int DFRobot_HT1632C::getCharOffset(int font_end [], byte font_index) {
	if(font_index == 0) {
		return 0;
	}
...
int DFRobot_HT1632C::getTextWidth(const char text [], int font_end [], byte font_height, byte gutter_space) {
	int wd = 0;
	byte i = 0;
	char currchar;
...
void DFRobot_HT1632C::setCursor(byte x, byte y){
	xIndex = x;
	this->setCursorIndex(x, y);
}

void DFRobot_HT1632C::setCursorIndex(byte x, byte y){
	xCoordinate = x;
	yCoordinate = y;
}

void DFRobot_HT1632C::setFont(byte value){
	fontValue = value;
}
...
void DFRobot_HT1632C::printNumber(unsigned long n, byte base){
...
void DFRobot_HT1632C::print(byte value, int base){
...
void DFRobot_HT1632C::print(double number, byte digits){
  int index = 0;
	char buf[10] = {'\0'};
...
void DFRobot_HT1632C::print(float number, byte digits){
...
void DFRobot_HT1632C::printStr(const char str[], byte value){
	if(fontValue == FONT8X4)
...
void DFRobot_HT1632C::print(const char str[], uint16_t speed){
	...
	this->printStr(str);	
	for(byte i=1; i<length+24;i++){	
		for(byte j = xIndex; j<24-xIndex; j++){
			matrix[j] = '\0';
		}
